<template>
	<view>
		<view class="wrap">
			...
			<view class="swiper-view">
				...
				<view class="swiper-wrap">
					<view class="swiper-img" v-for="(item,index) in list.slider" :key="index">
						<image src="../../static/index/sc.png" mode="aspectFill"
							style="width: 26rpx;height: 26rpx;position: relative;top: -80rpx;right: 0;z-index: 100;"
							@click="del(index)"></image>
						<image :src="item" mode="aspectFill" style="width: 120rpx;height: 100rpx;margin-right: 10rpx;">
						</image>
					</view>

					<view @click="uplodimage(2)" class="dcphoto">
						<image src="../../static/index/camera.png" mode="aspectFill"
							style="width: 120rpx;height: 100rpx;"></image>
					</view>
				</view>
			...
			<view class="input-wrap1" style="height: auto;" v-if="editorContentShow">
				<view>土地详情:</view>
				<rich-Text-Editor v-model="list.content" @input="input" style="height: auto;width: 500rpx;">
				</rich-Text-Editor>
				<!-- <rich-text :nodes="list.content" id="asdf"></rich-text> -->
			...
			<view class="input-wrap">
				<view>结束日期</view>
				<view class="uni-list-cell-db" style="width: 100%;height: 60rpx;">
					<picker mode="date" :value="gdate" @change="zhbindDateChange" fields="day">
						<view class="uni-input"
							style="margin-left: 20rpx;width: 90%;text-align: left;height: 60rpx;line-height: 60rpx;">
							{{gdate}}
						</view>
			...
			<view class="input-wrap">
				<view>结束时间</view>
				<view class="uni-list-cell-db" style="width: 100%;height: 60rpx;">
					<picker mode="time" :value="gtime" @change="zhbindDateChangeTime">
						<view class="uni-input"
							style="margin-left: 20rpx;width: 90%;text-align: left;height: 60rpx;line-height: 60rpx;">
							{{gtime}}
						</view>
	...
	export default {
		...
		methods: {
			...
			del(index) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							// 执行确认后的操作
							that.list.slider = that.list.slider.filter(item => item != that.list.slider[index])
			...
			getLandInfo(id) {
				this.$http.request({
					...
					if (res.data.code == 200) {
						...
						this.list.id = id;
						this.list.name = resdata.name;
						this.list.cover = resdata.cover;
						this.list.slider = resdata.slider;
						this.list.remark = resdata.remark;
						this.list.content = resdata.content;
						this.editorContentShow = true;
						this.list.start_price = resdata.start_price;
						this.list.add_price = resdata.add_price;
						this.list.auction_type = String(resdata.auction_type);
						this.choice = resdata.auction_type_name;
						this.date = resdata.start_time.slice(0, 10);
						this.time = resdata.start_time.slice(resdata.start_time.length - 8, resdata.start_time
							.length - 3);
						this.gdate = resdata.end_time.slice(0, 10);
						this.gtime = resdata.end_time.slice(resdata.start_time.length - 8, resdata.start_time
							.length - 3);
						this.list.delay_period = resdata.delay_period;
						this.list.province_id = resdata.province_id;
						this.list.city_id = resdata.city_id;
						this.list.region_id = resdata.region_id;
						this.list.town_id = resdata.town_id;
						this.list.village_id = resdata.village_id;
						this.list.address = resdata.address;
						this.list.acreage = resdata.acreage;
						this.list.land_cert = resdata.land_cert;
						this.list.plant = resdata.plant;
						// this.list.lat = resdata.lat;
						// this.list.lng = resdata.lng;

						this.choiceae = resdata.region[0] + resdata.region[1] + resdata.region[2] + resdata.region[
							3] + resdata.region[4];
						// this.list.content = this.con;
						console.log(this.list.content, "详情--reslog");
			...
			smit() {
				...
				console.log(typeof(auction_type), "auction_type0_0");
				if (name == '' || !name) {
